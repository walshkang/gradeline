...
from .grading import grade_one_submission, collect_locator_candidates, apply_locator_candidates, build_annotation_progress_callback, build_grading_progress_callback
from .preprocessing import compute_submission_pdf_hash, get_or_compute_preprocessing, compute_cache_key_for_submission, StageTiming, SubmissionTelemetry
...
class Orchestrator:
    ...
    def run(self, units: list[Any]) -> int:
        ...
        def preprocess_task(idx: int, unit: Any):
            try:
                extracted = get_or_compute_preprocessing(unit, self.config, self.diagnostics)
    ...
    def regrade_question(self, question_id: str, units: list[Any]) -> int:
        ...
        for unit in self.units:
            try:
                ...
                pre_extracted = get_or_compute_preprocessing(unit, self.config, self.diagnostics)
                combined_text = ""
                ...
                new_cache_key = compute_cache_key_for_submission(unit, self.config.rubric, self.config)
                payload = [
                    {
                        "verdict": qr.verdict,
